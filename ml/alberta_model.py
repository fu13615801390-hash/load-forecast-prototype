...
def _paths():
    ...
    models_dir = os.path.join(base_dir, "models", "alberta")
    return {
        "models_dir": models_dir,
        "model": os.path.join(models_dir, "alberta_test.keras"),
        "sx": os.path.join(models_dir, "scaler_alberta_x.save"),
        "sy": os.path.join(models_dir, "scaler_alberta_y.save"),
    }
...
def _resolve_model_loader():
    ...
    try:
        ...
        raise ImportError("No working Keras loader found for the Alberta model.") from e
...
def load_artifacts():
    ...
    if missing:
        raise FileNotFoundError(f"Missing Alberta model files in {p['models_dir']}: {missing}")
